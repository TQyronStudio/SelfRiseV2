...
## XP Progress Bar Design Standards

### Visual Design Requirements
**Implemented as standard for all XP progress components**

#### 1. Rarity-Based Color System
```typescript
// 6-tier color system based on level ranges (motivational progression)
Level 1-10:   Grey (#9E9E9E)     // Beginner
Level 11-20:  Green (#4CAF50)    // Growing 
Level 21-40:  Blue (#2196F3)     // Rare  
Level 41-60:  Purple (#9C27B0)   // Epic
Level 61-80:  Gold (#FFD700)     // Legendary
Level 81-100: Red (#F44336)      // Mythic/Exotic
```

#### 2. Components That Change Color Based on Level
**All these UI elements dynamically adapt their colors based on the user's current level:**

##### A) XP Progress Bar Component
- **Border**: 2px border around entire component
- **Roman numeral text**: Large serif numeral display
- **Level title text**: Serif level name above numeral
- **Progress bar gradient**: XP fill colors
- **Level badge**: Gradient background and border
...
#### 3. Motivational Color Progression
- **Grey (1-10)**: First steps, learning basics
- **Green (11-20)**: Growth phase, building momentum ✨ NEW 
...
#### 2. Roman Numeral Display System
```typescript
// Extract Roman numerals from level titles
const levelRomanAndTitle = useMemo(() => {
  const titleParts = levelInfo.title.match(/^(.+?)\s+([IVX]+)$/);
  return {
    baseName: titleParts[1], // e.g., "Beginner"
    romanNumeral: titleParts[2] // e.g., "V"
  };
}, [levelInfo.title]);
```

**Display Hierarchy:**
- **Top Center**: Level name + Roman numeral in monumentální serif style
- **Top Left Corner**: Level badge with breathing animation
- **Bottom**: XP progress bar and statistics

#### 3. Typography Standards - Roman Style
```typescript
// Level Title (above Roman numeral)
...
```typescript
// Column-based layout for vertical stacking
container: {
  flexDirection: 'column',
  position: 'relative', // For absolute badge positioning
}

// Badge positioning in top-left corner
...
// Roman text section at top center
...
#### 5. Animation Standards - Breathing Effect
```typescript
// All level badges have subtle breathing animation
const pulseAnimation = Animated.loop(
  Animated.sequence([
    Animated.timing(pulseAnim, {
      toValue: 1.04, // Subtle scale for organic feel
      duration: 2000, // Slow breathing rhythm
...
- **MANDATORY**: All XP progress components must follow this design standard
- **Colors**: Dynamic rarity-based border colors for all components
- **Typography**: Serif fonts for Roman numerals and level names
- **Animation**: Breathing effect for all level badges (not just milestones)
- **Layout**: Column structure with badge in top-left, Roman text top-center
- **Accessibility**: Proper labels and contrast ratios maintained

### Visual Hierarchy Priority
1. **Roman Numeral** (largest, most prominent)
2. **Level Name** (serif style, above numeral)
3. **Level Badge** (top-left corner, breathing animation)
4. **Progress Elements** (XP bar, percentages, statistics)
