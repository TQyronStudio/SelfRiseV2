...
## FÁZE 10: UI vrstva — plynulost a elegance tap animace (nástřel)

Tato fáze reaguje na symptom popsaný uživatelem 2026-04-20: *"Kliknu, vidím nějaký mezistupeň jako když se to promáčkne a potom změnu na finální verzi. Chybí plynulost a elegance."* Fáze 7–9 řeší **rychlost** (processing + dispatch), ale vizuální trhání může pramenit také z **animací a stylů**, které běží na JS threadu nebo přepočítávají layout. Cíl Fáze 10: z plynulé funkce udělat plynulý zážitek.

> ⚠️ **UPOZORNĚNÍ PRO CLAUDE**: Tato fáze je **diagnostická před implementační**. Nikdy nerefaktoruj UI naslepo — nejdřív změř (Profiler), pak cíleně oprav jeden viník. Plošná revize UI komponent je riziko regresí vzhledu a accessibility.

> ⚠️ **UPOZORNĚNÍ PRO CLAUDE**: Tato fáze se spouští **až po Fázi 9**. Důvod: Fáze 9 odstraní mezistupeň z trojitého dispatchu, což může sama o sobě eliminovat většinu pocitu "promáčknutí". Po Fázi 9 znovu otestovat a rozhodnout, zda Fáze 10 má ještě smysl.

---

### 10.1 Profiling: identifikace skutečného viníka laggu

**Problém:** Nevíme, která komponenta konkrétně při tapu re-renderuje, jak dlouho trvá její render, a která animace blokuje JS thread. Bez měření je optimalizace střelba naslepo.

**Postup řešení:**
1. Zapnout React DevTools Profiler (Flipper / standalone)
2. Nahrát "tap návyku" scénář (5 rychlých tapů)
3. Identifikovat:
   - Které komponenty re-renderují (a kolikrát)
   - Která komponenta má render time > 16 ms (blokuje 60 FPS)
   - Zda běží `LayoutAnimation` / `Animated` bez `useNativeDriver`
4. Zapsat zjištění jako 3–5 konkrétních viníků seřazených podle dopadu

**Status:** [ ] ČEKÁ

---

### 10.2 Checkbox animace — přepis na Reanimated worklets (pokud potřeba)

**Problém (hypotéza):** `HabitItemWithCompletion` checkbox animace pravděpodobně používá standard React Native `Animated` API s `useNativeDriver: false` (kvůli animaci barvy pozadí nebo layout property). To znamená, že každý frame animace běží přes JS bridge = trhání při tapu, když JS thread dělá i další práci.

**Postup řešení:**
1. Najít checkbox animaci v `HabitItemWithCompletion` (pravděpodobně `Animated.timing` nebo `spring`)
2. Pokud animuje layout/background color → přepsat na Reanimated `useSharedValue` + `useAnimatedStyle`
3. Zachovat stejné timing a easing curve (zachovat aktuální "feel")
4. Test: tap návyk → animace plynulá, žádné "promáčknutí"

**Status:** [ ] ČEKÁ

---

### 10.3 XP progress bar animace — ověřit native driver

**Problém (hypotéza):** `OptimizedXpProgressBar` po tapu animuje přírůstek XP. Pokud animace běží na JS threadu (`useNativeDriver: false`), blokuje render habit itemu.

**Postup řešení:**
1. Otevřít `OptimizedXpProgressBar.tsx` — zkontrolovat `useNativeDriver` flag
2. Pokud false → zkusit změnit na true (funguje pouze pro `transform` / `opacity`, ne pro `width`)
3. Pokud animuje `width` → přepsat na `transform: scaleX` s native driver
4. Test: tap návyk → XP bar se plní plynule, nepřerušuje animaci checkboxu

**Status:** [ ] ČEKÁ

---

### 10.4 Shadow/elevation optimalizace (Android)

**Problém (hypotéza):** Každý habit item má shadow (iOS) / elevation (Android). Shadow je drahý na re-render, zejména na Androidu. Pokud se při tapu re-renderuje celý seznam, shadow se přepočítává N-krát.

**Postup řešení:**
1. Ověřit profilerem, zda shadow re-render skutečně je hotspot
2. Pokud ano: přesunout shadow do `Animated.View` s `shouldRasterizeIOS={true}` nebo použít statický `Image` s pre-rendered shadow
3. Alternativa: snížit `elevation` z vysoké hodnoty na 2–3 (Android GPU cost klesá nelineárně)
4. Test: tap 5× rychle na Androidu → žádné frame drops

**Status:** [ ] ČEKÁ

---

### 10.5 XP notification banner — LayoutAnimation audit

**Problém (hypotéza):** Po tapu vlítne "+25 XP" banner shora. Pokud používá `LayoutAnimation` nebo `Animated.spring` bez native driveru, blokuje render habit itemu.

**Postup řešení:**
1. Najít komponentu banneru (`XpAnimationContext` nebo `XpNotificationBanner`)
2. Zkontrolovat animaci vjezdu/výjezdu
3. Přepsat na Reanimated pokud běží na JS threadu
4. Test: banner se zobrazí plynule současně s checkbox animací, ne postupně
...
| 10.1 | Profiling UI (diagnostika) | Nulové | 1 hodina |
| 10.2 | Checkbox → Reanimated | Střední | 1 hodina |
| 10.3 | XP bar native driver audit | Nízké | 30 minut |
| 10.4 | Shadow/elevation optimalizace | Nízké | 30 minut |
| 10.5 | Notification banner audit | Nízké | 30 minut |

**Očekávané výsledky:**
- Po Fázi 7: **~30–40 % zrychlení** (nulové riziko)
- Po Fázi 8: **~70 % zrychlení** (střední riziko, nutné testování)
- Po Fázi 9: **~80 % zrychlení** + odstranění mezistupně (vyšší riziko, volitelné)
- Po Fázi 10: plynulost a elegance animací (diagnostika napřed, pak cílené opravy)

**Celkový odhad bez Fáze 9+10:** 5–6 hodin práce včetně testování.
