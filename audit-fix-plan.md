...
**Status:** [x] HOTOVO — `SQLiteHabitStorage.createCompletion` a `toggleCompletion` rozšířeny o volitelný `completionId?: string`. `HabitsContext.toggleCompletion`: CREATE větev nyní pre-generuje `generateUUID()` a dispatchuje **1× ADD_COMPLETION** s plným payloadem (včetně `completedAt`, `isConverted: false`, `createdAt`, `updatedAt`). Temp_id swap + 2 dodatečné dispatche odstraněny. Rollback při storage failure = `DELETE_COMPLETION(preGeneratedId)`. DELETE větev nezměněna v počtu dispatchů (už byl 1), ale přidán `loadHabits()` fallback při storage failure pro resync. TS compile clean.

---

## FÁZE 10: UI vrstva — plynulost a elegance tap animace

Tato fáze reaguje na symptom popsaný uživatelem 2026-04-20: *"Kliknu, vidím nějaký mezistupeň jako když se to promáčkne a potom změnu na finální verzi. Chybí plynulost a elegance."*

Fáze 7–9 řeší **rychlost** (processing + dispatch). Po Fázi 9 uživatel potvrdil, že zrychlení je cítit, ale "mezistupeň" je stále vidět → problém není (jen) v datech, ale v **animační vrstvě**.

### 📋 Shrnutí diagnózy (provedeno 2026-04-21)

Místo abstraktního "profiling first" byl proveden **cílený audit kódu** (HabitCompletionButton, HabitItemWithCompletion, OptimizedXpProgressBar, XpNotification, XpAnimationContext). Závěry:

| # | Viník | Dopad | Důkaz |
|---|---|---|---|
| 🥇 | `HabitCompletionButton` **nemá ŽÁDNÉ animace** | ⭐⭐⭐⭐⭐ | [HabitCompletionButton.tsx:60-66](src/components/habits/HabitCompletionButton.tsx#L60-L66): `isAnimating` switchuje opacity + scale **skokem** bez `Animated.Value` / `withTiming` |
| 🥈 | `OptimizedXpProgressBar` animuje `width` bez native driveru | ⭐⭐⭐ | [OptimizedXpProgressBar.tsx:454](src/components/gamification/OptimizedXpProgressBar.tsx#L454): `useNativeDriver: false` na 500ms timing každého tapu |
| 🥉 | `isToggling` state v `HabitItemWithCompletion` → 2 re-rendery per tap | ⭐⭐ | [HabitItemWithCompletion.tsx:89,149,159](src/components/habits/HabitItemWithCompletion.tsx#L89): `setState(true)` → `setState(false)` bez UX významu díky optimistic UI z Fáze 9 |
| 4️⃣ | 7+ `console.log` per render v produkci v `OptimizedXpProgressBar` | ⭐ | [OptimizedXpProgressBar.tsx:50,109,126,438,442,471,536](src/components/gamification/OptimizedXpProgressBar.tsx#L50) |
| ✅ | XP notification banner (`XpNotification.tsx`) | — | Už používá native driver správně všude (řádky 270, 276, 281, 287, 292, 306, 311) — **netřeba řešit** |
| ✅ | Shadow/elevation | — | `HabitItemWithCompletion` nemá shadow; `OptimizedXpProgressBar` má `elevation: 5` jen v `bold` theme — **netřeba řešit** |

### 🤖 Claude Design skilly — verdikt

`frontend-design` skill je **web-focused** (Tailwind, Next.js, HTML artifacts) a nehodí se pro React Native animace. `ui-designer:ui-designer` agent má stejný web bias. Pro Reanimated 3 v RN je cennější pracovat přímo s konkrétním kódem, který už umíme přesně číst.

> ⚠️ **UPOZORNĚNÍ PRO CLAUDE**: Všechny body Fáze 10 jsou **pouze vizuální vrstva** — žádný z nich se nedotýká storage, state managementu, ani XP/gamifikační logiky. Make-up systém a Smart Bonus Conversion jsou **plně chráněny** (čtou z `state.completions` / `xp_transactions` — vrstvu pod UI). Nicméně smoke test Make-upu **zůstává povinný** po každém bodě (regresní ochrana).

---

### 10.1 HabitCompletionButton — přepis na Reanimated 3 (TOP PRIORITA)

**Problém:** Komponenta **vůbec neanimuje**. Při tapu dochází ke třem skokovým změnám:
1. `isAnimating=true` (během toggle): opacity 1→0.5 + scale 1→0.9 **skokem** (žádný přechod)
2. `isAnimating=false` + `isCompleted=true`: návrat + výměna `<View>` (prázdný kruh) za `<Ionicons checkmark>` **skokem**
3. `backgroundColor` se mění `border`→`success` **skokem** přes React state

To je ten **"promáčknutý mezistupeň"** — nejde o logiku (Fáze 9 ji vyřešila), ale o chybějící plynulý přechod mezi stavy.

**Vysvětlení:** Reanimated 3 umí animovat na UI threadu (worklets) **i barvy** pomocí `interpolateColor`. Přepisem na `useSharedValue` + `useAnimatedStyle` získáme:
- Press-in feedback (scale 1 → 0.92) na UI threadu
- Smooth barva border + background (border → success) přes `interpolateColor`
- Spring entrance checkmarku (scale 0 → 1, opacity 0 → 1)

Výsledek: **jeden plynulý přechod** místo 3 skoků.

**Ovlivněné soubory:**
- `src/components/habits/HabitCompletionButton.tsx` (přepis)

**Postup řešení:**
1. Nahradit `import { Animated } from 'react-native'` za `import Animated, { useSharedValue, useAnimatedStyle, withSpring, withTiming, interpolateColor, useReducedMotion } from 'react-native-reanimated'`
2. Přidat `import * as Haptics from 'expo-haptics'` a `import { Pressable, AccessibilityInfo } from 'react-native'`
3. Zavést shared values:
   - `scale = useSharedValue(1)` — press-in/out feedback
   - `completionProgress = useSharedValue(isCompleted ? 1 : 0)` — 0 = uncompleted, 1 = completed
4. Extrahovat theme barvy do konstant PŘED worklet scope:
   ```tsx
   ...
   ```
5. Detekce reduced motion preference (Apple HIG compliance):
   ```tsx
   const reducedMotion = useReducedMotion(); // hook z Reanimated 3
   ```
6. `useEffect([isCompleted])` sleduje prop a přepíná progress:
   ```tsx
   completionProgress.value = reducedMotion
     ? (isCompleted ? 1 : 0)  // instant pro reduced motion
     : withSpring(isCompleted ? 1 : 0, { damping: 15, stiffness: 200 });
   ```
7. `useAnimatedStyle` pro button container:
   ```tsx
   transform: [{ scale: scale.value }],
   backgroundColor: interpolateColor(completionProgress.value, [0, 1], [uncompletedBg, completedBg]),
   borderColor: interpolateColor(completionProgress.value, [0, 1], [uncompletedBorder, completedBorder])
   ```
8. `useAnimatedStyle` pro checkmark wrapper:
   ```tsx
   opacity: completionProgress.value,
   transform: [{ scale: completionProgress.value }]
   ```
9. `useAnimatedStyle` pro prázdný kruh wrapper (crossfade):
   ```tsx
   opacity: 1 - completionProgress.value
   ```
10. Nahradit `TouchableOpacity` za `Pressable`:
    - `onPressIn`: `Haptics.selectionAsync()` + `scale.value = withTiming(0.92, { duration: 80 })`
    - `onPressOut`: `scale.value = withSpring(1, { damping: 12, stiffness: 300 })`
    - Při `reducedMotion`: scale zůstane 1 (žádný press feedback)
11. **Accessibility props** (Apple HIG + Android a11y):
    ```tsx
    accessibilityRole="checkbox"
    accessibilityState={{ checked: isCompleted, disabled }}
    accessibilityLabel={...} // "Mark habit as complete" / "Mark habit as incomplete"
    ```
12. Odstranit prop `isAnimating` z API — nadále nepotřebný (optimistic UI z Fáze 9 + press-in animace pokrývají tactile feedback)
13. Renderovat checkmark i prázdný kruh SOUČASNĚ, oba wrapnuté v Animated.View s crossfade stylů (bod 8 + 9)

**Test:**
- Tap návyk → scale down (92%) + haptic tick, současně background morphuje šedá→zelená, checkmark fade+scale entrance
- Tap na už splněný → opačná animace
- 5× rychle po sobě → každý checkbox animuje samostatně, žádné frame drops
- **VoiceOver test (iOS)**: aktivovat VoiceOver → focus na checkbox → musí říct "Mark habit as complete, checkbox, unchecked" → po tapu "checked"
- **Reduced Motion test (iOS)**: Settings → Accessibility → Reduce Motion ON → tap → vše se přepne **instantně** bez spring (jen barva se skokem změní)

**💡 Volitelný delight enhancement (pokud čas + design review):**
- Checkmark entrance s overshoot: `withSpring(1, { damping: 10, stiffness: 180 })` → bounce 0 → 1.15 → 1 (pattern Duolingo, Apple Reminders)
- Micro-tilt při completion: krátká rotace 3° → 0° (pattern Things 3)

> ⚠️ **UPOZORNĚNÍ PRO CLAUDE**: `isAnimating` prop se aktuálně používá v [HabitItemWithCompletion.tsx:385](src/components/habits/HabitItemWithCompletion.tsx#L385). Po odstranění propu odstranit i `isToggling` state tamtéž (viz bod 10.3).

> ⚠️ **UPOZORNĚNÍ PRO CLAUDE**: `interpolateColor` v Reanimated 3 potřebuje hex/rgb barvy (ne theme keys). Proto krok 4 — extrahovat hodnoty MIMO worklet scope. Worklet nemůže číst non-shared values za hranicemi scope.

> ⚠️ **UPOZORNĚNÍ PRO CLAUDE**: `Pressable` nemá `activeOpacity` prop — press feedback řešíme přes scale animaci (krok 10). `disabled` prop funguje beze změny.

> ⚠️ **UPOZORNĚNÍ PRO CLAUDE**: `useReducedMotion` je dostupný v Reanimated 3.6+. Ověř verzi v `package.json` před implementací. Pokud není: fallback přes `AccessibilityInfo.isReduceMotionEnabled()` v useEffect.

> ⚠️ **UPOZORNĚNÍ PRO CLAUDE**: Haptic debounce z Fáze 7.3 je v `XpAnimationContext` (po úspěšném XP gain). Toto je JINÝ haptic — okamžitý na press-in, před storage call. Ne duplicitní, oba mají smysl (press feedback vs. reward feedback).

> ⚠️ **UPOZORNĚNÍ PRO CLAUDE**: `expo-haptics` by měl být už v projektu (kontrola: `grep expo-haptics package.json`). Pokud ne, neinstaluj — STOP a zeptej se.

> 🛡️ **OCHRANA MAKE-UP**: Komponenta je čistě prezentační — čte jen `isCompleted` prop z venku. Nevytváří, nečte ani neupravuje completion data. Make-up/Smart Bonus Conversion = nedotčené.

**Status:** [x] HOTOVO — `HabitCompletionButton` přepsán na `Pressable` + Reanimated 3: `scale` + `completionProgress` shared values, `interpolateColor` pro bg/border, crossfade checkmark/prázdný kruh, `useReducedMotion()` hook respektován (instant přepnutí), `Haptics.selectionAsync()` na `onPressIn`, `accessibilityRole="checkbox"` + `accessibilityState`. Prop `isAnimating` odstraněn. TS compile clean.

---

### 10.2 OptimizedXpProgressBar — migrace progress animace na Reanimated 3

**Problém:** Progress bar animuje `width` property přes RN `Animated` API s `useNativeDriver: false` ([řádek 454](src/components/gamification/OptimizedXpProgressBar.tsx#L454) a [463](src/components/gamification/OptimizedXpProgressBar.tsx#L463)). To znamená:
- Animace běží na JS threadu 200–500 ms per tap
- Současně běží React re-render, tap handler, XP event dispatch → trhání
- Komentář v kódu "Progress width requires layout thread" platil pro **staré** RN Animated API

**Vysvětlení:** Reanimated 3 umí animovat **libovolné style properties** (včetně `width`) na UI threadu přes worklets. Nepotřebujeme hack se `scaleX` transformem (který by navíc způsobil vizuální regresi gradientu — gradient by se rendroval přes celou šířku a pak se komprimoval, místo aby se roztahoval s fillem).

**Moderní přístup (2026, pattern z Facebook/Shopify RN apps):**
- `useSharedValue(0)` pro progress (0–100)
- `useAnimatedStyle` s `width: \`\${progress.value}%\``
- `useEffect` na změnu `xpProgress` triggeruje `progress.value = withSpring(...)` nebo `withTiming(...)`
- Animace běží na UI threadu, **bez JS bridge**, **bez visuálních kompromisů**

Výhoda oproti scaleX triku:
- ✅ Gradient se roztahuje přirozeně (žádná regrese)
- ✅ Žádný `transformOrigin` hack
- ✅ Native driver-like performance (worklet na UI threadu)
- ✅ Konzistentní s Fází 10.1 (stejný stack)

**Ovlivněné soubory:**
- `src/components/gamification/OptimizedXpProgressBar.tsx`

**Postup řešení:**
1. Přidat import: `import Animated, { useSharedValue, useAnimatedStyle, withTiming, withSpring, useReducedMotion } from 'react-native-reanimated';` (zachovat původní `Animated.Value` pro `pulseAnim` — ten už native driver má a funguje správně)
2. Nahradit `progressAnim = useRef(new Animated.Value(xpProgress)).current` za Reanimated shared value:
   ```tsx
   const progressShared = useSharedValue(xpProgress);
   ```
3. `useAnimatedStyle` pro progress fill:
   ```tsx
   const animatedProgressStyle = useAnimatedStyle(() => ({
     width: \`\${progressShared.value}%\`,
   }));
   ```
4. `useEffect([xpProgress, updateSequence])` — při změně cílové hodnoty:
   ```tsx
   const reducedMotion = useReducedMotion();
   const duration = reducedMotion ? 0 : (effectivePerformanceMode === 'performance' ? 200 : 500);
   
   if (reducedMotion || !animated) {
     progressShared.value = xpProgress;
   } else if (effectivePerformanceMode === 'performance') {
     progressShared.value = withTiming(xpProgress, { duration });
   } else {
     progressShared.value = withSpring(xpProgress, { damping: 15, stiffness: 100 });
   }
   ```
5. Odstranit starý throttling / `animationTimeoutRef` / `currentProgressRef` — Reanimated absorbuje rapid updates přirozeně (nový `withSpring` interrupt current animation)
6. Aplikovat `animatedProgressStyle` na `Animated.View` progress fill containeru
7. Ponechat `pulseAnim` (level badge breathing) beze změny — už má `useNativeDriver: true` a funguje
8. Test: tap návyk → XP bar plní plynule, gradient vizuálně identický s původním, current level badge pulzuje, checkbox animuje — vše současně bez trhání
9. **Reduced Motion test**: Settings → Accessibility → Reduce Motion ON → XP bar se přepne instantně bez animace

> ⚠️ **UPOZORNĚNÍ PRO CLAUDE**: Nepleť RN `Animated` (default import z 'react-native') s Reanimated `Animated` (z 'react-native-reanimated'). V komponentě budeme mít OBĚ — `pulseAnim` (RN Animated, native driver OK) a `progressShared` (Reanimated). Import aliasy:
> ```tsx
> import { Animated as RNAnimated } from 'react-native'; // zachovat pro pulseAnim
> import Animated from 'react-native-reanimated'; // default pro progress bar
> ```
> Nebo ponechat `Animated` jako Reanimated a pulseAnim refactorovat taky → **volba: ponechat minimální scope**, refactor jen progress fill.

> ⚠️ **UPOZORNĚNÍ PRO CLAUDE**: Level badge `pulseAnim` (`Animated.Value`) a jeho `Animated.View` wrapper musí zůstat **RN Animated** (ne Reanimated), protože používá `useNativeDriver: true` + `Animated.loop`. Refactor progress fillu se ho netýká.

> ⚠️ **UPOZORNĚNÍ PRO CLAUDE**: `updateSequence` dependency v useEffect je důležitá — donutí re-trigger animace i když `xpProgress` numericky stejné (např. level-up sequence kdy progress spadne z 95% na 5%).

> ⚠️ **UPOZORNĚNÍ PRO CLAUDE**: Reanimated 3 `withSpring` na percentage stringy (např. `'45%'`) NEPODPORUJE přímo — používej NUMBER shared value a interpoluj do stringu uvnitř `useAnimatedStyle` worklet.

> 🛡️ **OCHRANA MAKE-UP**: Progress bar čte XP stats z `GamificationService.getGamificationStats()` — nezapisuje nikam. Netýká se completion dat ani bonusů.

**Status:** [x] HOTOVO — progress fill migrován na Reanimated 3: `progressShared = useSharedValue(xpProgress)`, `useAnimatedStyle({ width: `${progress.value}%` })` na UI threadu. `withSpring({ damping: 15, stiffness: 100 })` pro quality mode, `withTiming(200ms)` pro performance mode, instant set pro reduced motion. Celá komplexní throttle/cache/ref logika odstraněna (Reanimated absorbuje rapid updates přirozeně). `pulseAnim` (level badge breathing) zachován jako `RNAnimated.Value` s native driverem. Gradient vizuálně beze změny. TS compile clean.

---

### 10.3 Odstranit `isToggling` state v HabitItemWithCompletion

**Problém:** Každý tap = `setIsToggling(true)` → re-render → `await toggle` → `setIsToggling(false)` → re-render. To jsou **2 zbytečné re-rendery** celého itemu per tap.

Stav měl dřív význam (zabránit double-tap během storage operace), ale po Fázi 9 má HabitCompletionButton **instant optimistic feedback** — přes prop `isCompleted` se překlopí hned. Guard proti double-tap není potřebný state → stačí `ref`.

**Vysvětlení:** `useRef` nespouští re-render. Stejná logika (ignoruj tap během běžící operace), ale nulový render impact.

**Ovlivněné soubory:**
- `src/components/habits/HabitItemWithCompletion.tsx`

**Postup řešení:**
1. Nahradit `const [isToggling, setIsToggling] = useState(false)` za `const isTogglingRef = useRef(false)`
2. `handleToggleCompletion`:
   - Guard: `if (isTogglingRef.current) return`
   - `isTogglingRef.current = true` (bez re-renderu)
   - `await onToggleCompletion(...)`
   - `finally { isTogglingRef.current = false }`
3. Odstranit prop `isAnimating={isToggling}` z `HabitCompletionButton` (konzistentní s bodem 10.1, kde prop odstraníme i z API)

**Test:** Tap → okamžitá reakce checkboxu (animace z 10.1), během animace druhý tap ignorovaný, po dokončení zase responzivní.

> 🛡️ **OCHRANA MAKE-UP**: Ref je čistě lokální guard — ne státní data. Completion data / Make-up = netknuté.

**Status:** [x] HOTOVO — `const [isToggling, setIsToggling] = useState(false)` nahrazeno `const isTogglingRef = useRef(false)`. Guard `if (isTogglingRef.current) return`, set/reset přes `.current = true/false` v try/finally. Prop `isAnimating={isToggling}` odstraněn z volání `HabitCompletionButton`. -2 re-rendery per tap.

---

### 10.4 Odstranit console.log spam z OptimizedXpProgressBar

**Problém:** Komponenta loguje na 7+ místech v každém renderu / každém eventu:
- Řádek 50: `'🎯 OptimizedXpProgressBar render'`
- Řádek 109, 126: cache invalidation logs
- Řádek 438, 442, 471: progress animation logs
- Řádek 536: performance log (v useEffect, tedy per update)

V produkčním buildu se console.log nevolá nativně, ale **string interpolation a template literal se vyhodnocují vždy** → zbytečná JS práce + garbage collection.

**Ovlivněné soubory:**
- `src/components/gamification/OptimizedXpProgressBar.tsx`

**Postup řešení:**
1. Buď odstranit všechny `console.log` (7 řádků) úplně
2. NEBO obalit: `if (__DEV__) console.log(...)` — při produkčním buildu se nevykoná ani interpolace
3. Ponechat `console.error` (řádky 100, 128) — chyby chceme vidět i v produkci

**Doporučení:** Odstranit úplně. Dev debug logy už splnily účel, produkční diagnostika se dělá přes Sentry/profiler, ne console.

**Status:** [x] HOTOVO — všechny `console.log` odstraněny (původní render log, level-up cache log, performance useEffect, memo comparator log). `console.error` zachovány pro produkci. `renderCountRef` + celý performance-logging useEffect odstraněny jako obsolete.
...
| 10.1 | HabitCompletionButton → Reanimated 3 + a11y + reduced motion + haptic | Nízké | 75 minut |
| 10.2 | XP progress bar: RN Animated → Reanimated 3 s animovaným width | Nízké | 45 minut |
| 10.3 | Odstranit isToggling state (useRef) | Nulové | 10 minut |
| 10.4 | Odstranit console.log z XP progress baru | Nulové | 10 minut |

**Očekávané výsledky:**
- Po Fázi 7: **~30–40 % zrychlení** (nulové riziko)
- Po Fázi 8: **~70 % zrychlení** (střední riziko, nutné testování)
- Po Fázi 9: **~80 % zrychlení** + odstranění mezistupně z logiky
- Po Fázi 10: **production-grade plynulost a elegance** — mezistupeň zmizí i vizuálně (checkbox plynule morphuje s haptic press feedback, XP bar animuje na UI threadu bez vizuální regrese, -2 re-rendery per tap, plná accessibility včetně Reduced Motion support)

**Celkový odhad Fáze 10:** ~2.5 hodiny práce včetně testování.
