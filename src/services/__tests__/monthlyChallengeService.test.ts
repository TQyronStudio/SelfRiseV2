...
describe('MonthlyChallengeService - Comprehensive Scenario Testing', () => {
  ...
  describe('Challenge Generation Scenarios', () => {
    ...
    test('Scenario 3: Moderate User - Partial Data Quality', async () => {
      ...
      expect(result.generationMetadata.warnings.some(w => 
        w.includes('partial') || w.includes('data quality')
      )).toBe(true);
    });
    ...
    test('Scenario 4: Category Variety Enforcement', async () => {
      ...
      expect(result.generationMetadata.warnings.some(w => 
        w.includes('repeating') || w.includes('repetition')
      )).toBe(true);
    });

    test('Scenario 5: Error Handling - Generation Failure Recovery', async () => {
      // Mock UserActivityTracker to throw error
      const mockUserActivityTracker = UserActivityTracker as jest.Mocked<typeof UserActivityTracker>;
      mockUserActivityTracker.calculateMonthlyBaseline.mockRejectedValue(new Error('Baseline calculation failed'));

      const result = await MonthlyChallengeService.generateChallengeForCurrentMonth('error_user');

      // Should still succeed with fallback
      expect(result.success).toBe(false); // Main generation failed
      expect(result.challenge).toBeDefined(); // But fallback challenge was created
      expect(result.challenge.title).toContain('Fallback:');
      expect(result.challenge.starLevel).toBe(1);
      expect(result.challenge.baseXPReward).toBe(300);
      expect(result.error).toContain('Challenge generation failed');
      expect(result.generationMetadata.warnings).toContain('Using fallback challenge');
    });
  ...
  describe('Edge Cases and Error Scenarios', () => {
    test('Storage Error Recovery', async () => {
      ...
      expect(result.challenge).toBeDefined(); // Fallback challenge created
